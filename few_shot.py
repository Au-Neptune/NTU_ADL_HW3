...
def perplexity(
    model, tokenizer, data, example, max_length=2048,
):
    data_size = len(data)
    examples = ""
    for i in range(len(example)):
        examples += f"{get_prompt(example[i]['instruction'])} {example[i]['output']}"
    instructions = [f"{examples}{get_prompt(x['instruction'])}" for x in data]
...
def select_data(data_path, num):
    with open(data_path, "r") as f:
        data = json.load(f)
    
    candidate_num = random.sample(range(0, len(data) + 1), num)
    candidate_data = []
    for i in candidate_num:
        candidate_data.append(data[i])
...
if __name__ == "__main__":
    ...
    # choose incontext learning data
    example = select_data(args.train_data_path, 2)

    with open(args.test_data_path, "r") as f:
        data = json.load(f)

    model.eval()
    ppl = perplexity(model, tokenizer, data, example)
